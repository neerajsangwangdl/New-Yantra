...
export class ProductInsertComponent implements OnInit, OnDestroy {
  ...
  get f() {
    return this.ProductInsertForm.controls;
  }
  ...
  getProductDetailsById() {
    this.productService.getProductDetailsById(this.productId).subscribe((p) => {
      this.product = p;
  ...
  getNewStockProducts() {
    this.productService.getNewStockProducts(this.productId).subscribe((p) => {
      this.newstockProducts = p;
      this.totalNewStock =
        this.globalService.getTotalOfColumn(p, 'newstock') || 0;
    });
  }
  ...
  handleImageUpload(event) {
    ...
    imageCompression(imageFile, options)
      .then(function (compressedFile) {
        ...
        console.log(
          `compressedFile size ${compressedFile.size / 1024 / 1024 / 1000} MB`
  ...
  //patchInventroyProductForm() pending
  ...
  deleteInventory(product_id) {
    if (confirm('Remove this product from this Inventory?')) {
      this.productService.deleteInventory(product_id).subscribe((res) => {
  ...
  applyInventory() {
    ...
    this.productService.applyInventory(data).subscribe((p) => {
      if (p) {
  ...
  searchCustomer() {
    ...
    this.customerList = [];
    this.checkoutService.SearchCustomer(data).subscribe((res) => {
      if (res['status'] == 200) {
  ...
  searchProduct() {
    ...
    let productPagingObj: ProductPaging = new ProductPaging();
    this.productService.getProductList(filterObj).subscribe((a) => {
      productPagingObj = a.data as ProductPaging;
      if (productPagingObj.Products) {
        this.productList = productPagingObj.Products.length
          ? productPagingObj.Products
          : [{ ProductId: 0, Name: 'No Data' }];
  ...
  onSubmit() {
    ...
    ProductInsertFormRaw['DisplayPriority'] = environment.production
      ? ProductInsertFormRaw['DisplayPriority']
      : '1';
    ...
    formData.append(
      'DiscountPercentage',
      ProductInsertFormRaw['DiscountPercentage']
    );
  ...
  updateNewInventory(product) {
    let newStock = parseInt(
      window.prompt('Kitne or peice badhane hai? (type digits only)')
    );
